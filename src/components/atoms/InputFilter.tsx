...
const InputFilter=({onSearch}: {onSearch: (searchTerm: string) => void;}) => {
    ...
    return (
        <form action="" onSubmit={(e: FormEvent<HTMLFormElement>): void => e.preventDefault()}>
            <input className='border-solid border-2 placeholder:text-gray-500 outline-none bg-transparent border-border rounded-lg  h-10 w-56 text-sm text-texto px-2'
                type="text"
                value={searchTerm}
