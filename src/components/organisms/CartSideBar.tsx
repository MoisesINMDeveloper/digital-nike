...
const CartSidebar=({isOpen,onClose}: {isOpen: boolean; onClose: () => void;}) => {
    ...
    return (
        <>
            <div ref={sidebarRef} className={`fixed z-40 top-10 right-0 h-auto w-80 border-2 border-t-0 rounded-lg rounded-t-none border-border text-texto bg-white shadow-lg transition-transform transform ${isOpen? 'translate-x-0':'translate-x-full'}`}>
